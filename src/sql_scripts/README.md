...
### Проблема с отсутствующим столбцом owner_id

В коде предполагается, что в таблице `processing_results` существует столбец `owner_id`, но в текущей схеме базы данных этого столбца нет. Это приводит к ошибке:
...
В коде были добавлены обходные решения:
1. В `src/api/app.py` - закомментирован фильтр по `owner_id` в экспорте результатов
2. В `src/tasks/worker.py` - добавлена обработка ошибок при вставке данных

#### Постоянное решение

Для устранения проблемы необходимо добавить столбец `owner_id` в таблицу `processing_results`. Создайте SQL скрипт миграции:

```sql
-- 04_add_owner_id_to_processing_results.sql
-- =============================================
-- Скрипт: Добавление столбца owner_id в таблицу processing_results
-- Автор: [Имя автора]
-- Дата: [Текущая дата]
-- =============================================
...
/* 
Описание:
Добавление столбца owner_id в таблицу processing_results для хранения идентификатора владельца результата обработки

Входные данные:
Таблица processing_results без столбца owner_id

Выходные данные:
Таблица processing_results со столбцом owner_id

Зависимости:
Таблица processing_results должна существовать
*/

-- Добавляем столбец owner_id
ALTER TABLE processing_results 
ADD COLUMN owner_id INTEGER;

-- Добавляем комментарий к столбцу (опционально)
COMMENT ON COLUMN processing_results.owner_id IS 'Идентификатор владельца результата обработки';

-- Создаем индекс для улучшения производительности запросов по owner_id (опционально)
CREATE INDEX IF NOT EXISTS idx_processing_results_owner_id ON processing_results(owner_id);
```

После создания этого скрипта, поместите его в директорию `full-stack/DB/init/` и пересоздайте базу данных или выполните скрипт вручную на существующей базе данных.
