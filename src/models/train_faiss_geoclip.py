...
def encode_images_simple(
    image_paths: List[str], coordinates: Dict[str, Tuple[float, float]], batch_size: int = 32
...
def save_index_and_mapping(index: faiss.Index, mapping_df: pd.DataFrame, index_path: str, mapping_path: str):
    """
...
def main():
    ...
    parser.add_argument(
        "--sample-size", type=int, default=1000, help="Количество изображений для обработки (по умолчанию 1000)"
    )
    parser.add_argument("--batch-size", type=int, default=32, help="Размер батча для обработки (по умолчанию 32)")
    parser.add_argument("--debug", action="store_true", help="Включить debug режим с более подробным логированием")
    ...
    OUTPUT_INDEX_PATH = "data/index/faiss_index_fine_tuned_optimized.bin"  # Путь для сохранения индекса
    OUTPUT_MAPPING_PATH = "data/index/image_mapping_fine_tuned_optimized.csv"  # Путь для сохранения маппинга
