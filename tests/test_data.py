...
def config():
    ...
    return {
        "features": {"numeric_features": ["age", "income"], "categorical_features": ["gender", "education"]},
...
def test_preprocess_data(sample_data, config):
    ...
    processed_data = preprocess_data(
        sample_data, config["features"]["numeric_features"], config["features"]["categorical_features"]
    )
    ...
    processed_data_with_nan = preprocess_data(
        sample_data_with_nan, config["features"]["numeric_features"], config["features"]["categorical_features"]
    )
...
def test_split_data(sample_data, config):
    ...
    X_train, X_test, y_train, y_test = split_data(
        sample_data, "target", config["data"]["test_size"], config["data"]["random_state"]
    )
